...
This action provides PR-level inline annotations; the broader NAV suite now also enforces these patterns at agent runtime:

- **mcp-server-nav-language** — Pure regex MCP server (sub-10ms) enforcing the same pattern set as the `no-animal-violence-pre-commit` scanner this action uses. Used by Gary MCP hub Phase 3. Reviewdog catches violations in PR diffs; the MCP server catches them in agent-generated content before it reaches a PR.
